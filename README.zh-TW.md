```markdown
[簡體中文](https://github.com/gebilaowang66662/ESLyric-full-screen-imitation-IOS-configuration/blob/main/README.zh-CN.md)
...
便携式版本，找到包含 `eslyric-data\layout` 的 `profile` 文件夾：  
...
以下設定可以根據個人偏好進行調整。為了達到上圖所示的效應，可以參考以下調整 👣
由於每個人的電腦螢幕大小不同 🖥💻 和數值可能會有所不同，您可以根據需要進行調整以達到類似的視覺效果。 🔧
因為外部介面和全螢幕介面有獨立的設定，您應該先進入全螢幕模式 🔳
...
雙擊 ESLyric 介面以切換/退出全螢幕。如果您不喜歡雙擊，您可以設定全螢幕模式的热鍵 🔳 或添加播放 ▶ 暫停 ⏸ 熱鍵
...
如果找不到，檢查安裝目錄中的 `profile` 文件夾，其中包含：

    `eslyric-data\scripts `
    
![b9bdf403918fa0ec4115922e