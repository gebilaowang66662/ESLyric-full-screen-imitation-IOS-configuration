...
**ESLyric 1.0.6.1** で **内蔵フローティングウィンドウ** が追加されたため、今回の更新ではレイアウト切り替えボタンが **プラグインの内蔵フローティングウィンドウ** を開くようになります。

以前の外部フローティングウィンドウを使用したい場合は、フローティングウィンドウ切り替えを含むレイアウトを開き、次の項目を置き換えてください：

* **`Floating Lyric`** → **`Closeflowin`**（フローティングウィンドウレイアウト）
* **`Floating Lyric`** → **`Showflowin`**（foobar の下部バーおよび縦レイアウト）
...
![O_ HN O@_NL8Y3_P NPG](https://github.com/user-attachments/assets/7f22d864-c1b7-4820-8086-5c3c1759425e)
 ...
 3種類のスペクトラム表示モード（ESLyric 1.0.6.5+）
...
設定パネルのパラメータを変更することで、スペクトラムの表示スタイルや挙動をカスタマイズできます。
